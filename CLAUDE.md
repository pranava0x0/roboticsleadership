...
- **A JS layout that measures "how much room do I have?" must not let the measured element's own content feed into the answer.** A flex item with `flex: 1 1 auto` sizes from its content, so measuring it *while it is over-full* reports the width it is demanding, not the width it will settle at. The priority+ nav read `nav.clientWidth` as **317px** with all nine links inline and **289px** once six had moved into the dropdown and its siblings relaxed — it fitted the strip to a width that no longer existed, and printed the last link underneath the overflow button. Use `flex: 1 1 0` (zero basis → width is a pure function of the siblings, stable across the whole pass), pin the siblings with `flex: 0 0 auto`, and give any element whose width you *reserve* `flex: 0 0 auto` too — an unpinned reserve element is already squeezed at the moment you sample it (`.primary-more` measured ~60px against a true 83px). Both bugs were invisible except in a narrow band of viewport widths.
- **`offsetWidth` reads 0 on a `[hidden]` element** — unhide it *before* sampling the width you intend to reserve, not after.
- **Summing `offsetWidth` across flex children silently under-counts by the gaps.** `.nav-list` and the nav both set `gap: 2px`, so a nine-item sum came back 718 against a true `scrollWidth` of 733 — enough to declare a fit that overflows by ~18px. Measure the container's `scrollWidth`, which includes gaps, and re-read it after each removal rather than subtracting a cached item width.
- **When a layout's behaviour depends on which element is "current", sweeping widths on ONE page proves nothing — sweep pages, not just widths.** The priority+ nav's restore appended parked items back onto the tail of the list, which only reproduces the authored order when every survivor sits at the front. Because the collapse loop pins the *current page's* link, any page whose link isn't first ended up permanently reordered (`companies.html` → `Front page | Companies | News | US vs China | …`). `index.html`'s link is index 0, so appending happened to be correct there, and a four-width sweep on the front page saw nothing. Snapshot the authored order once at init and rebuild from that array.
- **A "not connected" integration is a transient state to retry, not a capability to declare unavailable.** The Chrome extension returned "not connected" twice, so the X `PhysicalAI` harvest was reported blocked and the rest shipped without it — then it connected on the first retry and ran fine. The tool's own error text says retries usually fix it; the second failure was read as confirmation instead of as a second sample. Retry at least once after doing other work before telling anyone a capability is unavailable, and say what you retried if you still report it blocked.
- **A defect gets promoted by a redesign long before it gets fixed by one.** 729 of 785 news records carried a scraper config id (`hacker-news-robotics`) where a masthead belonged, for the life of the project, because it rendered in a dense card footer. The front-page rebuild moved that field under a lead headline and it became the first thing you read. When a field is promoted to a more prominent position, audit its values across the whole corpus first.
- **A fallback onto the old wrong value re-arms the bug you just fixed.** `source.publication || source.id` keeps a misconfigured feed producing valid records — and silently reproduces the exact defect for any feed added without the new field. `validate.js` has no per-entry schema for `sources.json`, so it would pass CI *and* the deploy gate. The scraper now warns once per source; a real schema check is in backlog.
- **An accessibility audit is only as good as its background set — state the set, not just the result.** The theme rework was checked against `--bg` and `--surface` and claimed "every (token, background) combination". Adding `--surface-2` — which carries nav hover, `aria-current`, and the "Sections" caret — turned up four sub-AA pairs, and a fifth on `--surface`. Every text token needs checking against all three surfaces it can land on.
- **The in-app Browser pane doesn't paint a backgrounded tab, so `requestAnimationFrame` never fires and every measurement reads 0.** A `javascript_tool` probe right after `navigate` reported `window.innerWidth: 0` and an unmoved nav, which looked exactly like "my resize handler is broken." It wasn't — a `computer{action:"screenshot"}` first forces the tab to render, and the same probe then returns real numbers. Screenshot before you measure anything layout-dependent, and don't debug a rAF-driven feature off a pre-paint read.
...
- **`index.html` is a news front page, not a dashboard (rebuilt 2026-07-28).** Masthead → a well holding the lead story and three secondaries → a rail of briefs → the data sections (numbers, scoreboard, capital, themes, companies), each under a `.section-title.rule` keyline. Two contracts hold it together: (a) **the three story positions come from one call to `RT.frontPageStories(news)`**, which slices a single sorted pass into `{lead, top, briefs}` — never call `latestNews` separately per position or the same story renders twice, silently, and only a reader notices (`render-static.test.js` asserts the ids are distinct); (b) every position has a bake slot (`index-lead` / `index-topstories` / `index-briefs` / `index-standfirst`), so adding a front-page block means adding a renderer to `app.js` **and** a slot to `indexSlots()` — the bake fails loud on a marker with no renderer or a renderer with no marker, which is the design.
- **The front page fetches `data/news-recent.json` (latest 60 + total), never `news.json` (added 2026-09-26).** `render-static.js` writes it on every run (gitignored — a derived artifact, not source); `RT.loadNewsRecent()` falls back to building the same payload from `news.json` when it is missing. Anything on `index.html` that needs the whole archive (a count, a date range) must take it from `recent.total`, not `news.length` — `renderStandfirst` and `renderNewsBriefs` already do. `layout-guards.test.js` fails the build if the page goes back to `RT.loadAll()`. Result: index 242 → 117 KB gzip; `news.html` (239 KB) is now the worst page and still needs the whole archive.
- **Long pages ship collapsed and let the controls open what they point at (2026-09-26).** `energy.html` application areas are `details.collapsible-section` (first one open); the jump chips open their target on click (a chip for the current hash fires no `hashchange`), and the filter opens matching sections while it is active and restores each section's prior state when cleared. `policies.html` caps the executive table at 15 rows (Show more / Show all), same shape as `companies.html`. A listener that passes `render` directly hands the Event in as `keepShown`; wrap it as `() => render()` or the cap never resets.
- **The primary nav is priority+: every section link ships once, inline, and `app.js` moves the trailing ones into the "Sections" dropdown as the window narrows.** Do not reintroduce a hand-picked "these three live under More" split, and do not duplicate the link list into the dropdown markup — `initResponsiveNav` moves the actual `<li>` nodes, so `aria-current`, listeners and focus order survive and a crawler still sees exactly one `<a>` per section. Three invariants: the current page's link is never moved (so you can always see where you are, even at 375px); `highlightActiveNav()` must run **before** `initResponsiveNav()` because it marks that link (`init()` owns the order); and with JS off nothing may be unreachable, which is why the default CSS leaves the strip horizontally scrollable and the `.nav-managed` class (added by JS) is what hands overflow to the dropdown.
- **The thesis is the editing knife.** "Where AI meets the physical world." (tagline changed 2026-08-14 from "America has the AI. China has the scale. Robotics is where they meet." — owner call, the old strap read too declarative; the full thesis statement in improvement-plan.md § The thesis is unchanged.) Every new chart, page, or dataset must answer "does this help a reader judge where the two countries actually stand?" — if it doesn't, it's decoration (see improvement-plan.md's cut list for precedents).
...
- **Collapsible `<details>` state is bound once, globally, and re-applied per render — never per-element on `DOMContentLoaded` (rewired 2026-07-21).** `initCollapsibleSections` binds a **single delegated `toggle` listener on `document` in the capture phase** (`toggle` does not bubble, but capture still reaches the target's ancestors), so persistence survives any container re-render. State restore/default lives in the idempotent `RT.applyCollapsibleState()`, which is a pure **restore/persist** layer: with no saved state it respects each section's **authored `open` attribute** and never force-opens or force-closes. **Any page that re-paints a container holding `details.collapsible-section` must call `RT.applyCollapsibleState()` right after** — china.html does, after `RT.paint('#vs-sections', …)`; without it the re-paint drops the persistence and leaves whatever the render authored. "Collapse by default to cut scroll" therefore lives in the **markup**, not JS: author `open` only on sections meant to start open (china's renderer opens just its first via `${i===0?'open':''}`; policies opens only the two filter-driven tables). A runtime "force first open" flag was tried and reverted — it force-opened energy's authored-closed below-the-fold shortlist (Codex review, PR #126). The old code bound on a `DOMContentLoaded` race before JS-rendered sections existed and did nothing (issues.md 2026-07-16).
...
- **On a JS-virtualized feed (X/Twitter and similar infinite-scroll SPAs), programmatic `scrollTo`/`scrollBy` moves the scroll position but does not reliably trigger the next-page fetch — and a tight loop of it plus dispatched synthetic events can freeze the tab (2026-07-27).** `window.scrollY` updated correctly, but the article count stayed flat across repeated bottom-of-page checks; a follow-up loop mixing `scrollTo` + dispatched `WheelEvent`s caused a 45s `Runtime.evaluate` timeout and left the tab blank. Real input — a `computer`-tool scroll action — loaded new content reliably every time. Prefer simulated real input over programmatic scroll APIs on feeds you don't control the source of; treat a stalled DOM count as a signal to switch input methods, not to loop harder on the same one.
- **Before treating a manually-sourced find as new data, check whether the automated pipeline already has it (2026-07-27).** A curated X bookmarks + PhysicalAI-list sweep surfaced ~8 strong-looking candidates; cross-checking by keyword against `news.json` showed 6 of 8 were already ingested under different record ids by the RSS/search-API scrapers. The cheap keyword grep before authoring anything is what caught it — skipping it would have produced duplicate records with different ids for the same event.
