...
- **`validate.js` is the only thing CI runs, and no workflow runs on a pull request.** `pages.yml` triggers on `push: branches: [main]` (not `pull_request`), so `gh pr checks` reports "no checks" on every PR and `validate.js` only runs *post-merge* — where the most it can do is fail the deploy on data that already landed. `npm test` runs nowhere at all. So: (a) anything you want enforced has to live in `validate.js`, which is why the curation gate went there; (b) the `*.test.js` suite is a local-only net — **run `npm test` by hand before shipping**, nothing else will; (c) don't add an assertion that depends on live record counts expecting CI to police it; (d) don't read a green PR as "CI passed" — there is no CI on PRs. (Backlogged: add a `pull_request` job.)
- **`_requires_curator_review` marks *which scraper ingested a record*, not whether it's any good.** It reads like a quality flag and isn't: the WS0 sweep found off-topic records on both sides of it (88 flagged in `policies.json`, plus ~29 equally off-topic records that were never flagged). Don't treat "unflagged" as "reviewed", and don't treat `robotics_scope` as a relevance signal either — it was auto-filled boilerplate ("Federal Register — robotics-related") on 118 of 155 records, so it contains the word "robotic" no matter what the record is about. Any relevance filter keyed on either field is measuring nothing.
