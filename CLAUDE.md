...
- **`validate.js` is the only thing CI runs; `npm test` runs nowhere.** `pages.yml`, `scrape-news.yml`, and `scrape-policy.yml` all call `node scripts/validate.js` and nothing else. So (a) anything you want *enforced* has to live in `validate.js` — that's why the curation gate went there, and why it can block a deploy; (b) the `*.test.js` suite is a local-only safety net that will rot unnoticed, so run `npm test` by hand before shipping; (c) don't put an assertion that depends on live record counts into the suite expecting CI to police it — nothing will. (Backlogged: add `npm test` to `pages.yml`.)
