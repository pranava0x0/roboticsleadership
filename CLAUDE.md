...
- **Collapsible `<details>` state is bound once, globally, and re-applied per render — never per-element on `DOMContentLoaded` (rewired 2026-07-21).** `initCollapsibleSections` binds a **single delegated `toggle` listener on `document` in the capture phase** (`toggle` does not bubble, but capture still reaches the target's ancestors), so persistence survives any container re-render. State restore/default lives in the idempotent `RT.applyCollapsibleState()`. **Any page that re-paints a container holding `details.collapsible-section` must call `RT.applyCollapsibleState()` right after** — china.html does, after `RT.paint('#vs-sections', …)`; without it the re-paint resets the sections to their authored `open`. The default is data-driven: `<body data-collapse-sections>` (china/energy/supply-chain) means "first section open, rest collapsed" on all viewports; unmarked pages respect each section's authored `open` attr. The old code bound on a `DOMContentLoaded` race before JS-rendered sections existed and did nothing (issues.md 2026-07-16).
