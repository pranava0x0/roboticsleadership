...
- **Regression test:** `scripts/lint-actions.js` now asserts both scraper workflows search beyond `gh pr list`'s default 30 results and reject fork-owned branches before checkout. YAML parsing, SHA-pinning, and the day-count/clamp checks also pass; the full branch-append path will be exercised for real on the next scheduled run while a PR is open.
