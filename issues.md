...
- **Compounding effect:** because each `auto/*` branch is cut fresh off `main` (not chained off the prior day's branch), every day's PR independently re-discovers the *entire* backlog since `main` last moved, not just that day's increment — so the 29 PRs are heavily overlapping, not additive. Latest branches (`auto/news-2026-09-07`, `auto/policy-2026-09-07`) already contained the full accumulated diff. The same pattern recurred twice more before the process fix landed: 6 PRs piled up 2026-09-11→09-15, 5 more 2026-09-17→09-21 (curated via #218) — confirming this was a structural gap, not a one-off.
- **Fix (data, 2026-09-07):** took the latest `auto/news-*` and `auto/policy-*` branches (already reflecting the full backlog), curated them by hand (dropped 28 of 45 new news candidates and all 3 new policy candidates as noise/duplicates/hobbyist-HN/stale reposts), and opened one clean PR against `main`. The 29 stale `auto/*` PRs were closed as superseded once that PR merged.
- **Fix (process, 2026-09-21):** `scrape-news.yml` and `scrape-policy.yml` now check `gh pr list` for an already-open `auto/news-*` / `auto/policy-*` PR before creating a new one. If one is open, the run checks out that branch instead of cutting a fresh one off `main`, and widens the scraper's `--days` lookback to cover however long the PR has been waiting (clamped 3–45 days for news, 7–60 for policy) instead of using the fixed 3/7-day default — so a lagging curation pass no longer causes either a pile of redundant PRs *or* a silent gap in coverage once curation catches up. At most one `auto/news-*` and one `auto/policy-*` PR can be open at a time now. Also deleted all 69 stale, already-closed `auto/*` branches cluttering the branch list (this session's `git push origin --delete` worked fine — a claim elsewhere that it 403s appears to have been specific to that session's auth setup, not a repo-wide restriction).
- **Regression test:** none automated — this is a scheduling/workflow-orchestration behavior, not something a schema check can exercise. Verified by reading the resulting YAML with `python3 -c "import yaml; yaml.safe_load(...)"`, `node scripts/lint-actions.js` (SHA-pinning), and a standalone arithmetic check of the day-count/clamp logic; the branch-append path itself will only be exercised for real on the next scheduled run while a PR is open.
