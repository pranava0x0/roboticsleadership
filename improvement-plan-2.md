...
- [x] **Bake step at deploy ("bake, not build")**: `scripts/render-static.js`, run inside pages.yml between validate and upload. It loads the JSON and injects rendered HTML into marked slots (`<!-- static:scoreboard -->` … ) in the shipped pages. The repo stays no-build for local dev (pages still hydrate client-side exactly as today; JS re-renders into the same containers). Renderers are already string-builders (renderNewsCard etc.), so most run in Node with a thin RT shim. Scope, in value order: **china.html** (BLUF + 20-4-4 tally + all 28 metric rows), **news.html** (latest ~50 cards), **index.html** (KPIs + top-5 news), **policies/states/companies** tables. Success bar: no-JS visible content ≥70% on those pages. **L — the flagship item of this plan**
  - **Done 2026-07-16 for china/news/index; tables deferred.** Measured no-JS visible text, before → after: **china 8% → 99%**, **news 12% → 94%**, **index 36% → 97%** — 71KB of HTML that crawlers previously could not see. All three clear the ≥70% bar, so policies/states/companies tables are now the *only* unbaked pages and can go in a follow-up rather than blocking WS1's remaining items.
  - **The premise was half-wrong, and that was the real work.** "Renderers are already string-builders" held for `renderNewsCard`, but china.html's and index.html's rendering lived in each page's **inline `<script>`** — invisible to a Node bake. So the bulk of this was moving that logic into `app.js` as pure DOM-free `RT.*` functions (`renderChinaSections`, `computeKPIs`, `renderThemeCards`, …), which the pages and the bake now share. china.html's inline script went 79 lines → 10; index.html's ~130 → ~30. That refactor, not the injection machinery, is what made the bake possible — and it's the discipline WS8 already asked for ("keep renderers portable"), arrived at from the other direction.
  - **Deviation from the sketch above: news bakes 20 cards, not ~50.** The feed paginates at `RT.NEWS_PAGE_SIZE`, so 50 baked cards would be replaced by 20 the moment JS ran, under a "Page 1 of 38" control contradicting what shipped. Matching the client's first paint keeps the deployed HTML honest and still hits 94%.
  - **Charts stay client-only.** `renderProductionTrend` writes into live SVG nodes and the valuation chart is a local closure. An SVG line chart carries no text a crawler reads beyond axis labels, so it wasn't worth the refactor in this pass.
  - **`ci.yml` shipped alongside it, not as a bonus but as a prerequisite.** The bake runs at deploy, and `pages.yml` is push-to-main only — so a bake bug would have surfaced *after* merge by taking the deploy down. The `pull_request` job (backlogged separately) is what moves that failure pre-merge, and it now runs the repo's tests for the first time anywhere.
